@echo off
echo ============================================
echo Starting Langfuse v2.50.0 (Stable Release)
echo ============================================

echo.
echo Step 1: Stopping and removing any existing containers...
docker stop langfuse-server langfuse-postgres 2>nul
docker rm langfuse-server langfuse-postgres 2>nul

echo.
echo Step 2: Pulling Langfuse v2.50.0 (known stable version)...
docker pull langfuse/langfuse:2.50.0

echo.
echo Step 3: Creating Docker network...
docker network create langfuse-net 2>nul

echo.
echo Step 4: Starting PostgreSQL database...
docker run -d ^
  --name langfuse-postgres ^
  --network langfuse-net ^
  -e POSTGRES_DB=langfuse ^
  -e POSTGRES_USER=langfuse ^
  -e POSTGRES_PASSWORD=langfuse ^
  -p 5432:5432 ^
  -v langfuse_postgres_data:/var/lib/postgresql/data ^
  --restart unless-stopped ^
  postgres:13

echo Waiting for PostgreSQL to start...
timeout /t 15 /nobreak

echo.
echo Step 5: Starting Langfuse v2.50.0 server...
docker run -d ^
  --name langfuse-server ^
  --network langfuse-net ^
  -e DATABASE_URL=postgresql://langfuse:langfuse@langfuse-postgres:5432/langfuse ^
  -e NEXTAUTH_SECRET=your-secret-key-change-in-production ^
  -e SALT=your-salt-key-change ^
  -e TELEMETRY_ENABLED=false ^
  -e NEXTAUTH_URL=http://localhost:3000 ^
  -p 3000:3000 ^
  --restart unless-stopped ^
  langfuse/langfuse:2.50.0

echo.
echo Step 6: Waiting for Langfuse to start...
timeout /t 20 /nobreak

echo.
echo Step 7: Checking container status...
echo.
docker ps --filter "name=langfuse"

echo.
echo Step 8: Checking Langfuse logs...
echo.
docker logs langfuse-server --tail=10

echo.
echo ============================================
echo Setup Complete!
echo ============================================
echo.
echo Langfuse v2.50.0 should be available at: http://localhost:3000
echo.
echo This version definitely doesn't require ClickHouse!
echo.
pause
